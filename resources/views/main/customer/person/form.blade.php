    ...
    <div class="box {{ CommonHelper::getBoxTheme() }}">
        ...
        <div class="box-body">
            ...
            <div class="form-group {{ $errors->has('is_banned') ? ' has-error' : '' }}">
                ...
                <div class="col-sm-8">
                    <select id="is_banned" name="is_banned" class="select2 form-control">
                        <option selected disabled>-- Choose Status --</option>
                        <option value="1" {{ (int)CommonHelper::getVal($model, 'is_banned') == 1 ? 'selected' : null }}>Banned</option>
                        <option value="2" {{ (int)CommonHelper::getVal($model, 'is_banned') == 2 ? 'selected' : null }}>Active</option>
                    </select>
                    @if ($errors->has('is_banned'))
                        <span class="help-block">
                            <strong>{{ $errors->first('is_banned') }}</strong>
                        </span>
                    @endif
                </div>
            </div>
